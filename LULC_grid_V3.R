...
create_LULC <- function(city, epsg){
  ...
  path <- paste0("~/Desktop/LULC-V3/", city)
  ...
  for (i in 1:length(city_grid$ID)){
    ...
    ee_drive_to_local(task = ee_monitoring(esa_img, max_attempts = 1000), dsn = here(path, "ESA_1m.tif"))
    ...
    # esa_rast <- esa_img %>%
    #   ee_utils_future_value() %>%
    #   ee_drive_to_local(here(path, dsn = here(path, "ESA_1m.tif")), overwrite = TRUE)
    ...
    # Combine OSM open space with ParkServe
    parkserve <- ee$FeatureCollection("projects/wri-datalab/USA_ParkServe_Parks")$
      filterBounds(bb_ee)$
      map(function(feature){
        feature$set('Value', 10)})$
      reduceToImage(properties = list('Value'), reducer = ee$Reducer$first())$
      reproject(paste0("EPSG:", epsg))
    
    # Download ESA to local via Google Drive
    parks_img <- ee_image_to_drive(parkserve,
                                   description = "parks_1m",
                                   timePrefix = FALSE,
                                   region = bb_ee,
                                   scale = 1,
                                   maxPixels = 10e10)
    ...
    # Move from drive to local
    ee_drive_to_local(task = ee_monitoring(parks_img, max_attempts = 1000), 
                      dsn = here(path, "parks_1m.tif"))
    
    # Delete from drive
    drive_rm("rgee_backup/parks_1m.tif")
    
    parks_rast <- rast(here(path, "parks_1m.tif"))
    ...
    open_space <- rast(here(path, "open_space_1m.tif"))
    
    open_space_rast <- max(parks_rast, open_space)
    
    writeRaster(x = open_space_rast, 
                filename = here(path, "open_space_1m.tif"),
                overwrite = TRUE,
                datatype = 'INT1U')
    ...
    # Save to GCS
    openBuilds <- ee$FeatureCollection("projects/sat-io/open-datasets/VIDA_COMBINED/USA")$
      filterBounds(bb_ee) %>% 
    ...
    # turn spherical geometries back on
    # sf::sf_use_s2(TRUE)
    ...
    tree <- readRDS(here("data", "V2-building-class-tree.rds"))
    ...
    buildings <- buildings %>% 
      mutate(Class = case_when(ULU == 1 ~ "non-residential",
                               ULU == 2 ~ "residential",
                               is.na(ULU) ~ "unclassified"),
             Value = case_when(Class == "unclassified" & Slope == "low" ~ 46,
                               Class == "non-residential" & Slope == "low" ~ 45,
                               Class == "residential" & Slope == "low" ~ 44,
                               Class == "unclassified" & Slope == "high" ~ 43,
                               Class == "non-residential" & Slope == "high" ~ 42,
                               Class == "residential" & Slope == "high" ~ 41)) %>% 
      select(Value)
    ...
    assetID <- paste0("projects/earthengine-legacy/assets/projects/wri-datalab/cities/SSC/LULC_V3/USA_", city, "_LULC_1m_", i)
